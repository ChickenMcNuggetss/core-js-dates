...
function getDayName(date) {
  ...
  const dateInner = new Date(new Date(date).toJSON()).toUTCString().slice(0, 3);
  return arrDays.find((el) => el.substring(0, 3) === dateInner);
}
...
function getNextFriday(date) {
  ...
  const toNextFriday = (12 - day) % 7;
  date.setDate(date.getDate() + (toNextFriday || 7));
...
function getCountDaysInMonth(month, year) {
  let num = month;
  num -= 1;
  return new Date(year, num + 1, 0).getDate();
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const result = Math.abs((firstDate - secondDate) / (1000 * 60 * 60 * 24)) + 1;
  return result;
}
...
function isDateInPeriod(date, period) {
  ...
  if (incomeDate >= startDate && incomeDate <= endDate) return true;
  return false;
}
/**
...
function formatDate(date) {
  const dateMonth = new Date(date).getUTCMonth() + 1;
  const dateDate = new Date(date).getUTCDate();
  const dateYear = new Date(date).getUTCFullYear();
  const dateHour =
    new Date(date).getUTCHours() > 13
      ? Math.abs(new Date(date).getUTCHours() + 1 - 13)
      : new Date(date).getUTCHours();
  const dateMinutes =
    new Date(date).getUTCMinutes() < 10
      ? `0${new Date(date).getUTCSeconds()}`
      : new Date(date).getUTCMinutes();
  const dateSeconds =
    new Date(date).getUTCSeconds() < 10
      ? `0${new Date(date).getUTCSeconds()}`
      : new Date(date).getUTCSeconds();
  const AMorPM =
    new Date(date).getUTCHours() <= 23 && new Date(date).getUTCHours() > 11
      ? 'PM'
      : 'AM';
  return `${dateMonth}/${dateDate}/${dateYear}, ${dateHour}:${dateMinutes}:${dateSeconds} ${AMorPM}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let countOfWeekends = 0;
  for (let i = 1; i < lastDayOfMonth.getDate() + 1; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const firstDate = new Date(thisYear, 0, 1);

  while (firstDate.getDay() !== 1) {
    firstDate.setDate(firstDate.getDate() - 1);
  }

  const difference = Math.floor((date - firstDate) / (1000 * 60 * 60 * 24)) + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const [dateStart, monthStart, yearStart] = period.start
    .split('-')
    .map(Number);
  const startDate = new Date(yearStart, monthStart - 1, dateStart);

  const [dateEnd, monthEnd, yearEnd] = period.end.split('-').map(Number);
  const endDate = new Date(yearEnd, monthEnd - 1, dateEnd);
